...
算法说明:
    对每个空间位置 (x,y) 的时间曲线 I(x,y,t) 做 butter + filtfilt。
...
from scipy.signal import butter, sosfiltfilt  # 使用 sosfiltfilt 提升稳定性
...
def temporal_bandpass(
    ...
    """沿时间轴对 [T,H,W] 帧序列做带通滤波，返回同形状 float32 数组。"""
    ...
    if frames.shape[0] < 24:
        return _fft_bandpass(frames, fps, lowcut, highcut)
    nyquist = fps * 0.5
    ...
    sos = butter(order, [low, high], btype="band", output="sos")  # 二阶节系数
    return sosfiltfilt(sos, frames, axis=0).astype(np.float32)  # 使用二阶节更稳定


def _fft_bandpass(frames: np.ndarray, fps: float, lowcut: float, highcut: float) -> np.ndarray:
    """当帧数太少不适合 filtfilt 时，用 FFT 掩码完成带通滤波。"""

    freq = np.fft.rfftfreq(frames.shape[0], d=1.0 / fps)
    spectrum = np.fft.rfft(frames, axis=0)
    keep = (freq >= lowcut) & (freq <= highcut)
    spectrum[~keep] = 0
    return np.fft.irfft(spectrum, n=frames.shape[0], axis=0).astype(np.float32)
...
def run(
    ...
    output_dir = ulm_io.step_dir("human", "step01_bandpass")  # step01 输出目录
    output_path = output_path or (output_dir / "human_filtered.npy")  # 默认输出文件
    ...
    preview_idx = n_frames // 2  # 用中间帧避开 filtfilt 边界效应
